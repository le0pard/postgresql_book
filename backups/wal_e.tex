...
Данный бэкап лучше делать раз в сутки (например, добавить в crontab). На рис~\ref{fig:wal-e1}-\ref{fig:wal-e3} видно как хранятся бэкапы на S3. Бэкапы сжаты через lzop (хуже сжимает, чем gzip, но скорость сжатия намного быстрее). Чтобы уменьшить нагрузку на чтения с жесткого диска бэкапы отправляются через mbuffer (через опцию << --cluster-read-rate-limit>> можно ограничить скорость чтения, если это требуется).
