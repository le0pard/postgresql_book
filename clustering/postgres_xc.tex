...
\begin{enumerate}
\item Таблица <<users\_with\_hash>> распределяется по хешу значения из указаного поля в таблице (тут указано поле id) по обработчикам данных. Вот как распределились первые 15 значений:
...
\item Таблица <<users\_with\_modulo>> распределяется по модулю значения из указаного поля в таблице (тут указано поле id) по обработчикам данных. Вот как распределились первые 15 значений:
...
Подсчет суммы с групировкой по полю:
